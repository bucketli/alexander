...
public class StringUtil {
    ...
    public static byte[] escapeEasternUnicodeByteStream(byte[] src, String srcString, int offset, int length) {
        ...
        while (true) {
            if (srcString.charAt(strIndex) == '\\') {// write it out as-is
                ...
                if (loByte < 0) loByte += 256; // adjust for signedness/wrap-around
                out.write(loByte);// We always write the first byte
                if (loByte >= 0x80) {
                    if (bufIndex < (bytesLen - 1)) {
                        int hiByte = src[bufIndex + 1];
                        if (hiByte < 0) hiByte += 256; // adjust for signedness/wrap-around
                        out.write(hiByte);// write the high byte here, and increment the index for the high byte
                        bufIndex++;
                        if (hiByte == 0x5C) out.write(hiByte);// escape 0x5c if necessary
                    }
                } else if (loByte == 0x5c) {
                    if (bufIndex < (bytesLen - 1)) {
                        int hiByte = src[bufIndex + 1];
    ...
    /********
     * 删除末尾的分号，如果有多个分号一直删除到出现不是分号
     */
    private static final char SEMICOLON = ';';
    private static final char BLANK = ' ';

    public static String trimTailSemicolon(String str) {
        if (isEmpty(str)) {
            return str;
        }
        int i = str.length() - 1;

        while (i >= 0 && (str.charAt(i) == SEMICOLON || str.charAt(i) == BLANK)) {
            i--;
        }
        if (i < 0) {
            return "";
    ...
    /**
     * 读取'='前后的key和value id = 1 and name = test age=1
     * 
     * @return <id,1> <name,test> <age,1> 参数名都小写,不过滤引号和结尾的';'
     * @param key/value 值不允许出现{'=',' '},key不允许重复, sql末尾不要以';'结束,
     */
    public static Map<String, String> getParameter(String sql, int offset) {
            ...
            case '=':
                ...
                if (isStarted) {
                    ...
                    loop: while (offset < length) {
                        ...
                        //'='说明前面读到的变量是key值，后面马上读到的为value值
                        case '=':
                            key = sql.substring(startIndex, endIndex).trim().toLowerCase();
                            ...
                            while (offset < length) {
                                ...
                                case ' ':
                                    ...
                                    if (isStarted) {
                                        ...
                                        value = sql.substring(startIndex, endIndex).trim();
                                        map.put(key, value);
                                        break loop;
                                    }
                                    break;
                                default:
                                    ...
                                    if (isStarted && (offset == length - 1)) {
                                        //最后的一个 key /value 处理
                                        value = sql.substring(startIndex, length).trim();
                                        map.put(key, value);
                                        break loop;
                                    }
                                    break;
